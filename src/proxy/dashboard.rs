...
//! `/corrall/status` with `x-api-key`, which is what lets a browser request
//! through the auth gate.

pub const HTML: &str = r#"<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Corrall</title>
...
<h1>Corrall <span id="meta" class="dim"></span></h1>
<div id="err"></div>
...
async function tick(){try{const st=await api('/corrall/status');$('#err').textContent='';$('#auth').style.display='none';
 ...
 for(const a of st.accounts){const tr=document.createElement('tr');if(a.current)tr.className='cur';if(a.blocked)tr.classList.add('dim');
  ...
  const b=document.createElement('button');b.textContent='use';b.onclick=async()=>{await api('/corrall/switch',{method:'POST',body:JSON.stringify({account:a.id})});tick();};tr.lastChild.appendChild(b);tb.appendChild(tr);}
